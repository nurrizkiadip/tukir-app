...
jQuery(function($) {
	...
	$("#btn_contact_submit").click(function() {
		//get input field values
		const user_name       = $('input[name=f_name]').val() + ' ' + $('input[name=l_name]').val();
		const user_email      = $('input[name=email]').val();
		const user_telephone  = $('input[name=phone]').val();
		...
		if (proceed) {
			//data to be sent to server
			const post_data = {'userName':user_name, 'userEmail':user_email, 'userTelephone':user_telephone, 'userMessage':user_message};
			//Ajax post data to server
